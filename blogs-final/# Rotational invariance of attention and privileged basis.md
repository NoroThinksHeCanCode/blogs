...
As a linear algebra enthusiast a very cool property of attention computation that I came across when I was reading [Anthropic's  A Mathematical Framework for Transformer Circuits](https://transformer-circuits.pub/2021/framework/index.html), was the idea that attention computations lack a privileged basis. Ofcourse I had heard of a basis of a vector space but what does this have to do with attention, I thought.
...
An MLP layer is another bunch of linear transformations with its weight matrices. While looking at a residual stream there are two things an MLP head does it reads information from it and writes information into it. This can be represented as two weight matrices $W_{read}$ and $W_{write}$.
